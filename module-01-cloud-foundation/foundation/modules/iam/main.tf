...
# Attached only when scope_workload = false.
resource "aws_iam_role_policy" "workload_grant" {
  count = var.scope_workload ? 0 : 1
...
# Hardened grant: explicit least-privilege. No action ends in :*, so nothing is
# service-wide, and every Resource is pinned to the foundation's own resources
# by name prefix (the random suffix Terraform adds is covered by the trailing
# wildcard IN THE RESOURCE ARN, which is not the same as a wildcard action).
# Attached only when scope_workload = true.
resource "aws_iam_role_policy" "workload_grant_scoped" {
  count = var.scope_workload ? 1 : 0
  name  = "${var.name_prefix}-workload-grant"
  role  = aws_iam_role.workload.id
